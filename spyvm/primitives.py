...
def func(interp, s_frame, argcount):
    ...
    w_arg1 = s_frame.pop() # receiver

    w_vm_params = [interp.space.wrap_int(0)] * 71
    w_vm_params[39] = interp.space.wrap_int(constants.BYTES_PER_WORD)
    w_vm_params[40] = interp.space.wrap_int(interp.image.version.magic)
    w_vm_params[69] = interp.space.wrap_int(constants.INTERP_PROXY_MAJOR)
    w_vm_params[70] = interp.space.wrap_int(constants.INTERP_PROXY_MINOR)

    if argcount == 0:
        return interp.space.wrap_list(w_vm_params)

    arg2_w = s_frame.pop() # index (really the receiver, index has been removed above)
    if not isinstance(w_arg1, model.W_SmallInteger):
        raise PrimitiveFailedError
    if argcount == 1:
        if not 0 <= w_arg1.value <= 70:
            raise PrimitiveFailedError
        return w_vm_params[w_arg1.value - 1]
